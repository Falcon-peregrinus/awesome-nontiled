...
myawesomemenu = {
   ...
   { "Редактировать конфигурацию", editor_cmd .. " " .. awesome.conffile },
   { "Перезапуск", awesome.restart },
   { "Выход", function () awful.util.spawn_with_shell("killall -u `whoami`") end}
}
...
table.insert(menu_items, { "Открыть терминал", terminal, freedesktop.utils.lookup_icon({icon = 'terminal'}) })
...
globalkeys = awful.util.table.join(
    ...
    awful.key({modkey}, "Print", function () awful.util.spawn("scrot -u", false) end),
...
client.add_signal("manage", function (c, startup)
    ...
    function createbutton( _image, _callback )
        local thebutton = awful.button({}, 1,nil, _callback)
        local thewidget = awful.widget.button({ image =  _image } )
        local rbts = thewidget:buttons()
        -- Add fancy junk
        for k, v in pairs(rbts) do
            thebutton[#thebutton + 1]=v
        end
        thewidget:buttons(thebutton)
        return thewidget
    end
    ...
    awful.titlebar.add(c, { modkey = modkey,
        widget = {
        createbutton( awful.util.getdir("config").."/minimize.png", function() c.minimized=true end ),
        layout = awful.widget.layout.horizontal.rightleft
        }
    })
    -- Enable sloppy focus
